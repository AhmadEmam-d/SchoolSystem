...
export function StudentAnnouncements() {
  ...
  const fetchData = async () => {
    try {
      ...
      // عرض إعلانات الطالب فقط + الإعلانات العامة
      const studentAnnouncements = (announcementsRes.data || []).filter(a =>
        ['student', 'all'].includes(a.target?.toLowerCase())
      );
      ...
      setSummary({
        totalAnnouncements: studentAnnouncements.length,
        publishedCount: studentAnnouncements.filter(
          a => a.status?.toLowerCase() === 'published'
        ).length,
        draftCount: studentAnnouncements.filter(
          a => a.status?.toLowerCase() === 'draft'
        ).length,
        urgentCount: studentAnnouncements.filter(
