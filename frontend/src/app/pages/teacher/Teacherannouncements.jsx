...
export function TeacherAnnouncements() {
  ...
  const fetchData = async () => {
    try {
      ...
      // عرض إعلانات المعلمين فقط + الإعلانات العامة
      const teacherAnnouncements = (announcementsRes.data || []).filter(a =>
        ['teacher', 'all'].includes(a.target?.toLowerCase())
      );
      ...
      setSummary({
        totalAnnouncements: teacherAnnouncements.length,
        publishedCount: teacherAnnouncements.filter(
          a => a.status?.toLowerCase() === 'published'
        ).length,
        draftCount: teacherAnnouncements.filter(
          a => a.status?.toLowerCase() === 'draft'
        ).length,
        urgentCount: teacherAnnouncements.filter(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => navigate('/teacher/TeacherAddAnnouncement')}>
